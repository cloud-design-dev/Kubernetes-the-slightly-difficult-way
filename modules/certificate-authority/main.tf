...
resource "null_resource" "kubelet_client" {
  ...
  count      = 1
  provisioner "local-exec" {
    command = "cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=${var.workers[count.index].name},${var.workers[count.index].primary_network_interface[0].primary_ip[0].address} -profile=kubernetes ${var.workers[count.index].name}-csr.json | cfssljson -bare ${var.workers[count.index].name}"
  }
}
...
resource "null_resource" "kube_apiserver" {
  ...
  provisioner "local-exec" {
    command = "cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=10.96.0.1,${var.workers[count.index].primary_network_interface[0].primary_ip[0].address},${var.loadbalancer_fqdn},127.0.0.1,kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.svc.cluster.local -profile=kubernetes kubernetes-csr.json | cfssljson -bare kubernetes"
  }
}
...
resource "null_resource" "kube_config_cluster" {
  ...
  count      = 3
  provisioner "local-exec" {
    command = "kubectl config set-cluster kubernetes-the-hard-way --certificate-authority=ca.pem --embed-certs=true --server=https://${var.loadbalancer_fqdn}:6443 --kubeconfig=${var.workers[count.index].name}.kubeconfig"
  }
}
...
resource "null_resource" "kube_config_creds" {
  ...
  count      = 1
  provisioner "local-exec" {
    command = "kubectl config set-credentials system:node:${var.workers[count.index].name} --client-certificate=${var.workers[count.index].name}.pem --client-key=${var.workers[count.index].name}-key.pem --embed-certs=true --kubeconfig=${var.workers[count.index].name}.kubeconfig"
  }
}

# {var.workers[count.index].name}
...
resource "null_resource" "kube_config_context" {
  ...
  count      = 3
  provisioner "local-exec" {
    command = "kubectl config set-context default --cluster=kubernetes-the-hard-way --user=system:node:${var.workers[count.index].name}--kubeconfig=${var.workers[count.index].name}.kubeconfig"
  }
}
...
resource "null_resource" "kube_config_default" {
  ...
  count      = 3
  provisioner "local-exec" {
    command = "kubectl config use-context default --cluster=kubernetes-the-hard-way --kubeconfig=${var.workers[count.index].name}.kubeconfig"
  }
}
